...
function Question({
  ...
  const difficultyColor = getDifficultyStyling(difficulty);
  ...
  return (
    <div className="flex flex-col justify-center mx-2 mb-2 text-black max-w-full h-fit px-4">
      ...
      <div className="flex flex-row flex-wrap w-full rounded-md mt-1 justify-center">
        <span
          className={
            `inline-flex items-center rounded-full mr-2 mb-2 px-2 py-1 text-xs font-medium ring-1 ring-inset text-nowrap ${difficultyColor}`
          }
        >
          {difficulty}
        </span>
        {settings.showTags ? topics &&
          topics.map((t, id) => (
            <span
              key={id}
            ...
            >
              {t}
                ...
                >
                  ...
              </span>} {/* idea: left/right carrots for show/hide */}
      </div>
    </div>
  );
}

function getDifficultyStyling(difficulty: string) {
  if (difficulty === "Easy") {
